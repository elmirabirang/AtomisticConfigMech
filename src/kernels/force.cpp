...
template<int dim> Force <dim>::~Force() {}
template <int dim> Force <dim>::Force(double sigma, double epsilon) {
    this->sigma = sigma;
    this->epsilon = epsilon;
}

template<int dim> Point<dim> Force<dim>::BondForce(Atom <dim> atoma, Atom <dim> atomb) {
...
template<int dim> Point<dim> Force<dim>::MaterialBondForce(Atom <dim> atoma, Atom <dim> atomb) {
	this->atomi = atoma;
	this->atomj = atomb;
	Bond<dim> bond;
...
template <int dim> double Force<dim>::interatomicConfigForceValue(Atom<dim> atoma, Atom<dim> atomb) {
	this->atomi = atoma;
	this->atomj = atomb;
	Bond<dim> bond;
...
template<int dim> Point<dim> Force<dim>::CriticalMaterialBondForce(Atom <dim> atoma, Atom <dim> atomb,double cutR) {
	this->atomi = atoma;
	this->atomj = atomb;
	Bond<dim> bond;
...
template<int dim> Point<dim> Force<dim>::SpatialBondForce(Atom <dim> atoma, Atom <dim> atomb) {
	this->atomi = atoma;
	this->atomj = atomb;
	Bond<dim> bond;
...
template<int dim> Point<dim> Force<dim>::ConfigurationalForce(Atom<dim> *atoma) {
	this->atomi = *atoma;
    ...
    LOOP_OVER_ATOM_NEIGHBORS(neighbor, atomi,
   	    Atom <dim> *neigh;
        neigh = *neighbor;
...
template<int dim> double Force<dim>::EnergyRelease(Atom<dim> *atoma, double delta_x) {
	this->atomi=*atoma;
	double config_force;
...
template<int dim> Point<dim> Force<dim>::ResultantForce(Atom <dim> *atoma) {
    this->atomi = *atoma;
...
template<int dim> Point<dim> Force<dim>::ResultantConfigForce(Atom <dim>* atoma) {
	this->atomi = *atoma;
	Bond<dim> bond;
	double material_distance=bond.MaterialBondDistance(atomi,atomj);
...
template<int dim> double Force<dim>::DerivMaterialEnergy(Atom <dim> atoma, Atom <dim> atomb) {
	this->atomi = atoma;
	this->atomj = atomb;
	double material_distance=bond.MaterialBondDistance(atomi,atomj);
	double sigma0=1/material_distance;
...
template<int dim> Point<dim> Force<dim>::derivativeMorseFunction(Atom <dim>* atoma, double alpha1, double alpha2,
...
template<int dim> double Force<dim>::electronDensity(Atom<dim>* atoma, double a, double beta1,
...
template<int dim> Point<dim> Force<dim>::embeddingForce(Atom <dim> * atoma, double a, double beta1,
...
template<int dim> Point<dim> Force<dim>::configForceEamPair(Atom <dim>* atoma, double alpha1, double alpha2,
                                             double r_01, double r_02, double E1, double E2, double delta,
                                             double r_cut, double h, double r_s1, double r_s2, double r_s3,
                                             double s1, double s2, double s3) {
	Point <dim> atoma_force;
...
template<int dim> Point<dim> Force<dim>::configForceEamEmbedding(Atom <dim> * atoma, double a, double beta1,
                                                   double beta2, double r_03, double r_04, double F0, double F2,
                                                   double q1, double q2, double q3, double q4, double Q1, double Q2,
                                                   double h, double r_cut) {
	Force <dim> force;
...
template<int dim> Point<dim> Force<dim>::StillingerWeberThreeBodyForceH2H3(Atom <dim> &atom_Beta, Atom <dim> &atom_Alpha,
...
template<int dim> Point <dim> Force <dim>::StillingerWeberThreeBodyForceH1(Atom <dim> *atom_Beta, Atom <dim> *atom_Alpha,
                                                                     Atom <dim> *atom_Gamma, double sigma_AlphaBeta,
                                                                     double sigma_AlphaGamma,double epsilon_AlphaBetaGamma,
                                                                     double gamma, double lambda_AlphaBetaGamma,
                                                                     double cosine_teta0, double a_AlphaBeta, double a_AlphaGamma) {
...
template<int dim> Point<dim> Force<dim>::StillingerWeberTwoBodyForce(Atom <dim> &atom_Alpha, Atom <dim> &atom_Beta,
...
template<int dim> Point<dim> Force<dim>::ResultantSWTwoBodyForce(Atom <dim> *atoma, double sigma_AlphaBeta, double epsilon_AlphaBeta,
                                                 double A_AlphaBeta, double B_AlphaBeta, double p_AlphaBeta,
                                                 double q_AlphaBeta, double a_AlphaBeta) {
	this->atomi=*atoma;
	Point <dim> atoma_force;
...
template<int dim> Point<dim> Force<dim>::ResultantSWThreeBodyForce( Atom <dim> *atoma, double sigma_AlphaBeta,
                                                    double sigma_AlphaGamma,double epsilon_AlphaBetaGamma,
    ...
    LOOP_OVER_ATOM_NEIGHBORS_3BODY(Beta, Gamma, atomi,
...
template<int dim> Point<dim> Force <dim>::deformationalBondSWForce(Atom <dim> &atom_Alpha, Atom <dim> &atom_Beta,
                                                   double sigma_AlphaBeta, double epsilon_AlphaBeta,
...
template<int dim> Point<dim> Force<dim>::deformationalprSW3BodyForce(Atom <dim> &atom_alpha, Atom <dim> &atom_beta, Atom <dim> &atom_gamma,
...
template<int dim> Point<dim> Force<dim>::deformationalcpSW3BodyForce(Atom <dim> *atom_alpha, Atom <dim> *atom_beta, Atom <dim> *atom_gamma,
                                                     double sigma_AlphaBeta, double sigma_AlphaGamma,double epsilon_AlphaBetaGamma,
	                                                 double mu, double eta_AlphaBetaGamma,
                                                     double cosine_teta0, double a_AlphaBeta, double a_AlphaGamma) {
	this->atomi = *atom_alpha;
...
template<int dim> Point<dim> Force<dim>::configurationalBondSWForce(Atom <dim> &atom_alpha,Atom <dim> &atom_beta,
		                                            double sigma_AlphaBeta, double epsilon_AlphaBeta,
		                                            double A_AlphaBeta, double B_AlphaBeta, double I_AlphaBeta,
					                                double J_AlphaBeta, double a_AlphaBeta) {
	this->atomi = atom_alpha;
...
template<int dim> Point<dim> Force<dim>::configurationalcpSW3BodyForce(Atom <dim> *atom_alpha, Atom <dim> *atom_beta, Atom <dim> *atom_gamma,
		                                               double sigma_AlphaBeta,double sigma_AlphaGamma,double epsilon_AlphaBetaGamma,
									                   double mu, double eta_AlphaBetaGamma,
                                                       double cosine_teta0, double a_AlphaBeta, double a_AlphaGamma) {
...
template<int dim> Point<dim> Force <dim>::configurationalprSW3BodyForce(Atom <dim> *atom_alpha, Atom <dim> *atom_beta, Atom <dim> *atom_gamma,
		                                                           double sigma_AlphaBeta,double sigma_AlphaGamma,double epsilon_AlphaBetaGamma,
									                               double mu, double eta_AlphaBetaGamma,
                                                                   double cosine_teta0, double a_AlphaBeta, double a_AlphaGamma) {
...
template<int dim> Point<dim> Force<dim>::ResultantConfigSWTwoBodyForce(Atom <dim> *atoma, double sigma_AlphaBeta, double epsilon_AlphaBeta,
                                                       double A_AlphaBeta, double B_AlphaBeta, double p_AlphaBeta,
                                                       double q_AlphaBeta, double a_AlphaBeta) {
	this->atomi=*atoma;
...
Point <dim> Force <dim>::ResultantConfigSWThreeBodyForce( Atom <dim> *atoma, double sigma_AlphaBeta,
    ...
    LOOP_OVER_ATOM_NEIGHBORS_3BODY(neigh_beta, neigh_gamma, atomi,
        Point <dim> ThreeBodyForceH1(0.,0.,0.);
