...
template <typename T> ThirdTensor <T>::~ThirdTensor() {}

template <typename T> ThirdTensor <T>& ThirdTensor <T>::operator =(const ThirdTensor<T>& rhs) {
	if (&rhs==this)
		return *this;
...
template class ThirdTensor<double>;
