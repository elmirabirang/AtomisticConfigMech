...
public:
	...
	virtual bool Evaluate (const double* parameters, double* cost, double* gradient) const
	{
        ...
        if (gradient!=NULL)
        {
        	...
        	for(At atom=Atoms.begin(); atom!=Atoms.end(); ++atom)
        	{
        		...
        		if( atom_region!=1 && atom_region!=2 && atom_region!=3 )
        		{
            		...
            		gradient[iter_force]=-(0.01)*resultant_force.GetXCoord();
            		gradient[iter_force+1]=-(0.01)*resultant_force.GetYCoord();
    	...
    	cost[0]=0.01*energy.TotPotentialEnergy(Atoms);
//    	cout << cost[0] << "\n";
...
// 		ufile << atoms.size() << endl;
    ...
    string path2="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.cracktip";
    ofstream ffile;
	ffile.open(path2.c_str());

    string path3="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.totalenergy";
    ofstream Efile;
	Efile.open(path3.c_str());

    string path4="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.reactionforce";
    ofstream EFfile;
	EFfile.open(path4.c_str());

    string path5="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.neighbors";
    ofstream Nfile;
	Nfile.open(path5.c_str());

///////////////////////////////////////////////////////////////////////Energy Minimization//////////////////////////////////////////////////////////////////////////////////////
    double load_warmup=0;

    int load_step_warmup=0;
    int max_load_step_warmup=14;
    bool check_broken_bond=false;


    while (load_step_warmup < max_load_step_warmup)
    {
...
/////////////////////////////////////////////////////////////OutPut Texts Files//////////////////////////////////////////////////////////////////////////////////////////////
    ...
    string path="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.crack";
    string path1="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/spatial/blunting/large_displacement_step/configurational_force/warmup/dump.crackspatial_warmup" ;
    string path5="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.temporalmaterial";
...
//		Mfile << atoms.size() << endl;
		...
		Efile << load_warmup << " " << totalEnergy << endl;
		...
		for (At at=atoms.begin(); at!=atoms.end(); ++at)
		{
			...
			{
				Point <2> config_force =force.ResultantForce(*at);
    			Point <2> matposition = (*at)-> GetMaterialPosition();
    			...
    			if (atom_id==704)
    			{
    				...
    				ffile << load_warmup << " "  << force_norm << " " << crack_tip_potential_energy << endl;

        			file << id<< " " << "6" <<" " << materialp_x<<" "
        		    << setprecision(5)<< materialp_y <<" " << "0.0"<<
				    " "<< config_force_x << " " << config_force_y <<" " << "0.0"<<endl;

        			ofile << id<< " " << "6" <<" " << spatialp_x<<" "
        		    << setprecision(5)<< spatialp_y <<" " << "0.0"<<
				    " "<< config_force_x << " " << config_force_y <<" " << "0.0"<<endl;

        			Mfile << id<< " " << "6" <<" " << temporalp_x<<" "
	...
	string write_data_path="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests"
			               "/results/dim2/spatial/blunting/large_displacement_step/configurational_force/broken_bonds/trajectory/trajectory.data";

	writeDataFile <2> (atoms, load_step_warmup, write_data_path);
	...
	EFfile << load_warmup <<" " << reactionForce->PointNorm() << endl;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    ...
	    for (At atom=atoms.begin(); atom!=atoms.end(); ++atom)
	    {
	    	...
	    	if (atom_id==704)
	    	{
	    		...
	    		vector <Atom <2>*> bond_neighbors=(*atom)->BondNeighbor();

	    		int number_neighbors=neighbors.size()+bond_neighbors.size();

	    		Nfile<< load_step_warmup << "   "<< number_neighbors << endl;

	    		cout << "number_neighbors: "<< number_neighbors <<endl;
	    	...
	    	if( (*atom)->GetAtomRegion()==2 )
	    	{
	        	...
	        	Point <2> Pnewt;
	        	double Ynew= Yold + ( load_warmup/2);
        ...
        vector < Atom <2>* > unrelaxed_atoms;
        unrelaxed_atoms=UnrelaxedConfigGenerator <2> (50, 29, 0, 1.1, 2);

        Boundary <2> B_bottom;

        double B_BC_x_min=0.0;
        double B_BC_y_min=0.0;

        double B_BC_x_max=55.0;
        double B_BC_y_max=0.1;
        int B_BC_id=1;

        B_bottom.SetBoundaryRegion(B_BC_x_min, B_BC_y_min, B_BC_x_max,
        		                   B_BC_y_max, B_BC_id);

        Boundary <2> B_top;

        double T_BC_x_min=0.0;
        double T_BC_y_min=26.6;

        double T_BC_x_max=55.0;
        double T_BC_y_max=26.7;
        int T_BC_id=2;

        B_top.SetBoundaryRegion(T_BC_x_min, T_BC_y_min,T_BC_x_max,
        		                 T_BC_y_max, T_BC_id);

        vector < Boundary <2> > all_boundaries;

        all_boundaries.push_back(B_bottom);
        all_boundaries.push_back(B_top);
	...
	//    double crack_bottom_y_max=13.6;
	    ...
	    all_boundaries.push_back(crack_top);
	    all_boundaries.push_back(crack_bottom);
	    all_boundaries.push_back(crack);

	    typedef typename vector < Atom <2>* >::iterator At;
	    for (At atom=unrelaxed_atoms.begin(); atom!=unrelaxed_atoms.end(); ++atom)
	    {

	        AssignRegion(*atom,all_boundaries);

	    }

	    vector < Atom <2>* > restart_atoms=FindNeighbors(unrelaxed_atoms,1.5);

		vector < Atom <2>* > opt_atoms;

		int iteration=0;

	    for (At atom=restart_atoms.begin(); atom!=restart_atoms.end(); ++atom)
	    {

	    	Point <2> relaxed_position=atoms[iteration]->GetSpatialPosition();

	    	(*atom)->SetSpatialPosition(relaxed_position);
	    	(*atom)->SetMaterialPosition(relaxed_position);

	    	iteration+=1;

	    	if ( (*atom)->GetAtomRegion()==0 || (*atom)->GetAtomRegion()==4 || (*atom)->GetAtomRegion()==5)
	    	{
	            opt_atoms.push_back(*atom);
	    	}

	    }

	    int num_opt_atoms=opt_atoms.size()*2;

		int iter=0;
		int number_of_atoms=0;
		while (iter<num_opt_atoms)
		{

			Point <2> opt_atom=opt_atoms[number_of_atoms]->GetSpatialPosition();
			double x=opt_atom.GetXCoord();
			double y=opt_atom.GetYCoord();
	    ...
	    cout << "number of optimized atoms: " << num_opt_atoms << endl;

	    double load=0;

	    int load_step=0;
	    int max_load_step=27;
    	...
    	ceres::GradientProblem problem (new MolecularStatic(restart_atoms, 1.0, 1.0, num_opt_atoms));
    	ceres::GradientProblemSolver::Options options;
    	options.minimizer_progress_to_stdout=true;
    	options.max_num_iterations=1500;
    	options.gradient_tolerance=1e-6;
...
///////////////////////////////////////////////////////////////OutPut Texts Files//////////////////////////////////////////////////////////////////////////////////////////////
		    ...
		    string path="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.crack";
		    string path1="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/spatial/blunting/large_displacement_step/configurational_force/warmup/dump.crackspatial" ;
		    string path5="/calculate/elmira/Implementations/PyQC_C++/pyQC _C++ version/tests/results/dim2/material/blunting/large_displacement_step/configurational_force/warmup/dump.temporalmaterial";
		...
		//		Mfile << atoms.size() << endl;
				...
				for (At at=restart_atoms.begin(); at!=restart_atoms.end(); ++at)
				{
			...
			writeDataFile <2> (restart_atoms, load_step, write_data_path);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    ...
	    for (At atom=restart_atoms.begin(); atom!=restart_atoms.end(); ++atom)
	    {
	    	...
	    	if (atom_id==704)
	    	{
	    		...
	    		Nfile<< load_step << "   "<< number_neighbors << endl;
